...
using changedmod.Content.Items.Placeable.Furniture;
...
{
	public class HaydenPlush : ModTile
	{
		public override void SetStaticDefaults() {
			...
			RegisterItemDrop(ModContent.ItemType<Items.Placeable.Furniture.HaydenPlush>(), 1);
		...
		public override bool RightClick(int i, int j) {
			...
			if (Main.netMode == NetmodeID.MultiplayerClient && !isLocked) {
				...
				if (isLocked) {
					...
					int key = ModContent.ItemType<HaydenPlushKey>();
		...
		public override void MouseOver(int i, int j) {
			...
			if (chest < 0) {
				...
				if (player.cursorItemIconText == defaultName) {
					player.cursorItemIconID = ModContent.ItemType<Content.Items.Placeable.Furniture.HaydenPlush>();
					if (Main.tile[left, top].TileFrameX / 36 == 1) {
						player.cursorItemIconID = ModContent.ItemType<HaydenPlushKey>();
					}
